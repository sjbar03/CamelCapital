...
let log_r stock = 
  if stock.prev_close <> 0.0 then 
    log (stock.close /. stock.prev_close)
  else 0.0
...
let estimate_garch_params data =
  ...
  let rec optimize params best_score iteration =
    if iteration <= 0 then params
      ...
      let best_index, best_new_score = List.fold_left (fun (bi, bs) (i, s) ->
        if s < bs then (i, s) else (bi, bs)
      ) (0, best_score) (List.mapi (fun i s -> (i, s)) scores) in
      if best_new_score < best_score then
        optimize (List.nth tested_params best_index) best_new_score (iteration - 1)
      else
        params
  in
