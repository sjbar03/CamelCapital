...
let get_int_body (col : t) =
  match col with
  | IntColumn (_, b) -> b
  | _ -> raise WrongFunction

let get_float_body (col : t) =
  match col with
  | FloatColumn (_, b) -> b
  | _ -> raise WrongFunction
...
let optional_safe (func : 'a -> 'a) (x : 'a option) =
  if x = None then None else Some (func (Option.get x))

let map_int_col func col =
  IntColumn (title col, List.map (optional_safe func) (get_int_body col))

let map_float_col func col =
  FloatColumn (title col, List.map (optional_safe func) (get_float_body col))

let map_string_col func col =
  StringColumn (title col, List.map (optional_safe func) (get_string_body col))
...
let rec filter_indices_int col ?(acc = []) ?(index = 0) crit_func =
  match get_int_body col with
  | [] -> List.sort Int.compare acc
  | h :: t ->
      if crit_func h then
        filter_indices_int
          (IntColumn (title col, t))
...
let rec filter_indices_float col ?(acc = []) ?(index = 0) crit_func =
  match get_float_body col with
  | [] -> List.sort Int.compare acc
  | h :: t ->
      if crit_func h then
        filter_indices_float
          (FloatColumn (title col, t))
          crit_func ~acc ~index:(index + 1)
      else
        filter_indices_float
          (FloatColumn (title col, t))
...
let reduce_int_col col func init =
  match col with
  | IntColumn (_, b) -> List.fold_left func init b
  | _ -> raise WrongFunction

let reduce_float_col col func init =
  match col with
  | FloatColumn (_, b) -> List.fold_left func init b
  | _ -> raise WrongFunction

let reduce_string_col col func init =
  match col with
  | StringColumn (_, b) -> List.fold_left func init b
  | _ -> raise WrongFunction
